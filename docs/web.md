...
Correção do atalho no iPhone: sem teclado, a conversa usa `100dvh` e posição superior zero. A altura e o deslocamento de `VisualViewport` só são aplicados quando o campo de mensagem está focado e há redução de altura compatível com teclado; o deslocamento é limitado ao espaço da tela. A página externa fica fixa apenas na conversa em telas pequenas, com rolagem dentro do histórico. Foco, orientação, `pageshow` e retorno ao aplicativo recalculam o layout e descartam medidas antigas, preservando o rascunho. O teste `chat-viewport.spec.js` reproduziu o deslocamento de 380 px antes da correção e cobre fechamento do teclado, métricas inconsistentes, retorno e envio. A flag de instalação e as medidas são simuladas; a validação final do atalho requer um iPhone físico.
